  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
      ...
      <div className="mt-4 px-4 pb-4">
        <p className="mb-2 text-xs font-medium text-[var(--muted)]">По часам</p>
        <div className="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {station.hours.map((h, i) => (
            ...
            >
              ...
              <span className="mt-0.5 text-[10px] text-[var(--gold)]">дальних</span>
              <span className="text-[10px] text-white/80">всего {h.total}</span>
              <span className="mt-0.5 text-[10px] text-[var(--muted)]">Э {h.suburban}</span>
            </div>
          ))}
        </div>
      </div>
